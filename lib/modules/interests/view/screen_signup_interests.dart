...
class ScreenSignupInterests extends StatefulWidget {
  ...
  final List<int>? currentUserInterests;

  const ScreenSignupInterests({
    super.key,
...
class _ScreenSignupInterestsState extends State<ScreenSignupInterests> {
  ...
  // All interests from API
  List<ModelInterests> allInterests = [];

  // Filtered interests for display (based on search)
  List<ModelInterests> displayedInterests = [];

  TextEditingController searchController = TextEditingController();

  // This is our single source of truth for selected interests
  Set<int> selectedInterestIds = <int>{};
  ...
  void initState() {
    ...
    // Initialize selected interests with current user interests if provided
    if (widget.currentUserInterests != null) {
      selectedInterestIds.addAll(widget.currentUserInterests!);
    }
  ...
  void filterInterests() {
    setState(() {
      String searchText = searchController.text.toLowerCase().trim();

      if (searchText.isEmpty) {
        displayedInterests = List.from(allInterests);
      } else {
        displayedInterests = allInterests
            .where((interest) => (interest.interestName ?? '')
            .toLowerCase()
            .contains(searchText))
            .toList();
  ...
  Widget interestGridview() {
    return Wrap(
      ...
      spacing: Dimens.margin8,
      runSpacing: Dimens.margin8,
      children: displayedInterests.map((item) {
        final int interestId = item.id ?? 0;
        final bool isSelected = selectedInterestIds.contains(interestId);

        return GestureDetector(
          onTap: () => toggleInterest(interestId),
          child: Container(
            padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 8),
            decoration: BoxDecoration(
              color: isSelected
  ...
  Widget actionButton(BuildContext context) {
    String buttonText = widget.isFromSignup
        ? getTranslate(APPStrings.textNext)
        : "Update"; // You can add this to APPStrings
            ...
            Align(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    getTranslate(APPStrings.textArtistsAndCommunities).toString(),
                    style: getTextStyleFromFont(
                      AppFont.poppins,
                      Dimens.margin18,
                      Colors.black,
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          Expanded(
            child: displayedInterests.isEmpty && !isLoading.value
                ? Center(
              child: Text(
                getTranslate(APPStrings.textNoInterests),
                style: getTextStyleFromFont(
              ...
              listeners: [
                BlocListener<GetInterestsListsBloc, GetInterestsListsState>(
                  listener: (context, state) {
                    ...
                    if (state is GetInterestsListsResponse) {
                      setState(() {
                        allInterests = (state.modelInterestsList.data ?? [])
                            .map((interest) => ModelInterests(
                          id: interest.id,
                          interestName: interest.interestName,
                          interestColor: interest.interestColor,
                          isInterestAdded: interest.isInterestAdded,
                        ))
                            .toList();
  ...
  void validate() {
    if (selectedInterestIds.isEmpty) {
      ToastController.showToast(
        context,
        getTranslate(ValidationString.textValidateOneInterests),
        false,
      );
    } else {
      updateInterests();
  ...
  void updateInterests() {
    Map<String, dynamic> body = {
      AppConfig.paramUserId: getUser().userData?.id ?? 0,
      AppConfig.paramInterestsId: selectedInterestIds.toList()
    };
    BlocProvider.of<UpdateUserInterestsBloc>(context).add(
      UpdateUserInterests(url: AppUrls.apiUpdateUserInterests, body: body),
    );
