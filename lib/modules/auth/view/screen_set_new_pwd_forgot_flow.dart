...
class _ScreenSetNewPasswordState extends State<ScreenSetNewPassword> {
  ...
  final ValueNotifier<Map<String, bool>> passwordValidationNotifier = ValueNotifier({
    'minLength': false,
    'hasUpperLower': false,
  ...
  void initState() {
    super.initState();
    _choosePasswordController.addListener(_validatePassword);
  }

  @override
  void dispose() {
    _choosePasswordController.removeListener(_validatePassword);
  ...
  void _validatePassword() {
    ...
    passwordValidationNotifier.value = {
      ...
      'hasUpperLower': RegExp(r'(?=.*[a-z])(?=.*[A-Z])').hasMatch(pwd),
      'hasSpecialChar': RegExp(r'[!@#\$%^&*(),.?":{}|<>]').hasMatch(pwd),
      'noSpaces': !pwd.contains(' '),
    };
  }
  ...
  Widget passwordValidationCriteria() {
    return ValueListenableBuilder<Map<String, bool>>(
      ...
      builder: (context, validation, child) {
        return Column(
          ...
          children: [
            _buildValidationItem(validation['minLength']!, 'Minimum 8 characters'),
            _buildValidationItem(validation['hasUpperLower']!, 'Must include upper case and lower case letters'),
            _buildValidationItem(validation['hasSpecialChar']!, 'At least one special character (e.g., !@#\$%^&*)'),
            _buildValidationItem(validation['noSpaces']!, 'No spaces allowed'),
          ],
        );
      },
    );
      ...
      builder: (context, values, child) {
        return BlocListener<ChangeOrSetNewPwdBloc, ChangeOrSetNewPwdState>(
          ...
          child: BaseRoundedBackgroundWidget(
            appBarText: getTranslate(APPStrings.textChangePassword,),
            child: Scaffold(
