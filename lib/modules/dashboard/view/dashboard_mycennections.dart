...
class _MyConnectionsState extends State<MyConnections> {
        ...
        BlocListener<FavouritesBloc, FavouritesState>(
          listener: (context, state) {
            ...
            if (state is FavouritesResponse) {
              ...
              // Update the current user's favorite status in the list
              _updateUserFavoriteStatus(getUser().userData?.id ?? 0, true);
        ...
        BlocListener<RemoveFromFavouriteBloc, RemoveFromFavouriteState>(
          listener: (context, state) {
            ...
            if (state is RemoveFromFavouriteResponse) {
              ...
              // Update the current user's favorite status in the list
              _updateUserFavoriteStatus(getUser().userData?.id ?? 0, false);
        ...
        body: ValueListenableBuilder<bool>(
          ...
          builder: (context, loading, _) {
            ...
            return MultiValueListenableBuilder(
              valueListenables: [myConnections, myFavoriteConnections],
  ...
  void _updateUserFavoriteStatus(int userId, bool isFavorite) {
    // Update in regular connections list
    final regularConnections = myConnections.value;
    for (int i = 0; i < regularConnections.length; i++) {
      if (regularConnections[i].id == userId) {
        regularConnections[i].isFavourite = isFavorite;
        break;
      }
    }
    myConnections.value = List.from(regularConnections);

    // Update in favorites list
    final favoriteConnections = myFavoriteConnections.value;
    if (isFavorite) {
      // Find the user in regular connections and add to favorites if not already there
      final user = regularConnections.firstWhere(
            (conn) => conn.id == userId,
